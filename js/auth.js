...
window.supabaseClient.auth.onAuthStateChange(async (_, session) => {
  ...
  const legacyMenu = document.getElementById("userMenu") || document.getElementById("user-menu");
  if (legacyMenu) legacyMenu.classList.toggle("hidden", !window.currentUser);
