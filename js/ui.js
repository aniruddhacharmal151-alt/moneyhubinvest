...
window.createPlanCard = function createPlanCard(plan, loggedIn) {
  ...
  card.querySelector("button").onclick = () => {
    ...
    document.getElementById("deposit-plan-summary").innerHTML = `
      ...
      <div>Amount: ${window.formatCurrency(plan.amount)}</div>
      <div>Expected Total: ${window.formatCurrency(plan.totalReturn)}</div>
    `;
    document.getElementById("manual-deposit-amount").value = plan.amount;
